...
public class MainActivity extends Activity {
	...
	//存储着a,b,c,d,e这里字母在mListItems中的索引位置
	ArrayList<Integer> mListSectionPos;

	// array list to store listView data
	//存储着a,add,above,b,boy,buttom这些数据的集合
	...
	public class ListFilter extends Filter {
		//调用filter方法，传入的参数就是performFiltering函数中的constraint
		//mAdaptor.getFilter().filter(str);调用后会调用该方法
		@Override
		protected FilterResults performFiltering(CharSequence constraint) {
			// NOTE: 这个方法总是在后台线程中调用而不是UI线程
			//这里的constrainStr是用户输入的字母
			...
			if (constraint != null && constraint.toString().length() > 0) {
				...
				synchronized (this) {
					for (String item : mItems) {
						//如果当前的这个ListView的item条目是以constraintStr开头
		...
		/**
		 * 在调用performFiltering方法后，也就是在过滤完之后调用该方法。
		 * @param constraint
		 * @param results
		 */
		@SuppressWarnings("unchecked")
	...
	private class Poplulate extends AsyncTask<ArrayList<String>, Void, Void> {
		...
		/**
		 * 通过EditView中的内容过滤ITEMS中的条目并封装到集合中
		 * @param params	过滤后的条目
		 * @return
		 */
		@Override
		protected Void doInBackground(ArrayList<String>... params) {
			...
			//说明搜索有结果
			if (mItems.size() > 0) {
				// NOT forget to sort array，把搜索到的结果排序
				Collections.sort(items, new SortIgnoreCase());
				//遍历所有的筛选结果
				String prev_section = "";//记录当前的位置
